...
from find_api.models.feedback import PersonFeedback, GeneralFeedback, FeedbackType
...
@router.post("/people/{person_id}/feedback/split", response_model=PersonFeedbackResponse)
def submit_split_feedback(
    ...
    if len(faces) != len(body.face_ids):
        raise HTTPException(
            status_code=400,
            detail="One or more face_ids do not belong to this person"
        )
...
@router.post("/people/{person_id}/feedback/merge/{target_person_id}", response_model=PersonFeedbackResponse)
...
@router.post("/people/{person_id}/feedback/wrong-person", response_model=PersonFeedbackResponse)
def submit_wrong_person_feedback(
    ...
    if len(faces) != len(body.face_ids):
        raise HTTPException(status_code=400, detail="One or more faces don't belong to this person")
    ...
    try:
        ...
        logger.info(f"Marked {len(body.face_ids)} faces as wrong person for {person_id}")
...
@router.post("/people/{person_id}/feedback/correct", response_model=PersonFeedbackResponse)
...
def rate_search_result(
    ...
    if not body.media_id:
        raise HTTPException(status_code=400, detail="media_id is required for search rating")
    ...
    try:
        ...
        logger.info(f"Search rating recorded for media {body.media_id}: {body.rating} stars")
...
def rate_caption(
    ...
    """
    if not body.media_id:
        raise HTTPException(status_code=400, detail="media_id is required for caption rating")
    ...
    try:
        ...
        logger.info(f"Caption rating recorded for media {body.media_id}: {body.rating} stars")
...
def rate_object_detection(
    ...
    """
    if not body.media_id:
        raise HTTPException(status_code=400, detail="media_id is required for object rating")
    ...
    try:
        ...
        logger.info(f"Object rating recorded for media {body.media_id}: {body.rating} stars")
    ...
    try:
        ...
        return {
            ...
            "general_feedback": {
                ...
                "search_avg_rating": round(avg_search_rating, 2) if avg_search_rating else None,
                "caption_avg_rating": round(avg_caption_rating, 2) if avg_caption_rating else None,
                "object_avg_rating": round(avg_object_rating, 2) if avg_object_rating else None,
            },
        }
    except Exception as e:
