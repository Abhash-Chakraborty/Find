...
def unlock_vault(client, db, *, passphrase: str = "correct horse battery staple") -> str:
...
def hide_media(client, db, *, media: Media, token: str) -> Path:
    ...
    with (
        patch("find_api.routers.vault.get_file", return_value=get_valid_image_bytes()),
...
class TestVaultHide:
    ...
    def test_hide_happy_path(self, client, db):
        media = seed_media(db)
        token = unlock_vault(client, db)
    ...
    def test_duplicate_hide_rejected(self, client, db):
        media = seed_media(db)
        token = unlock_vault(client, db)
...
class TestVaultStream:
    ...
    def test_expired_session_token_rejected(self, client, db):
        media = seed_media(db)
        token = unlock_vault(client, db)
        encrypted_path = hide_media(client, db, media=media, token=token)
    ...
    def test_stream_happy_path(self, client, db):
    ...
    def test_tampered_gcm_tag_causes_decryption_error(self, client, db):
...
class TestVaultGalleryIntegration:
    ...
    def test_hidden_media_excluded_from_gallery(self, client, db):
