...
- [~] **All tests green** — unit + integration + component **all green and recorded** (backend **437** / frontend **209**; `tsc` + `ruff` clean; `next build` succeeds). **API-level E2E journey green** (`test_e2e_journey.py` — full surface, server-observable). **Browser E2E suite (§10.2) not built** (needs Playwright + a live stack).
